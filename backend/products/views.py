...
from .models import FooterSection, FooterLink, FooterSocialMedia, FooterSettings
from .serializers import (
    FooterSectionSerializer, FooterLinkSerializer,
    FooterSocialMediaSerializer, FooterSettingsSerializer
)


class FooterDataView(APIView):
    """Public endpoint to get all footer data"""
    permission_classes = [AllowAny]
    
    def get(self, request):
        sections = FooterSection.objects.filter(is_active=True).prefetch_related('links')
        social_media = FooterSocialMedia.objects.filter(is_active=True)
        ...
        return Response({
            ...
            'settings': FooterSettingsSerializer(settings).data if settings else None
        })


class FooterSectionViewSet(viewsets.ModelViewSet):
    """Admin CRUD for footer sections"""
...
class FooterSocialMediaViewSet(viewsets.ModelViewSet):
    """Admin CRUD for social media links"""
    queryset = FooterSocialMedia.objects.all()
    serializer_class = FooterSocialMediaSerializer
    permission_classes = [IsAdminUser]


class FooterSettingsViewSet(viewsets.ModelViewSet):
    """Admin CRUD for footer settings"""
